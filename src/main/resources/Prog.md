...
public class Test {
    ...
    private static void test1() throws InterruptedException {
        ...
        t1.start();
        t1.join(); // 等待线程 t1 执行结束，之后再输出 r 的值
...
<img src="./Images/Prog_Image/JUC-%E4%B8%A4%E9%98%B6%E6%AE%B5%E7%BB%88%E6%AD%A2%E6%A8%A1%E5%BC%8F.png" style="zoom: 67%;" />
...
- **线程**是操作系统中最小的执行单元。在 Java 中，线程就是程序运行时的一个单独的执行路径。
- 每个**线程**启动后，Java 虚拟机（JVM）都会为它分配一块**栈内存**，叫做**Java 虚拟机栈**。
- 这个栈内存中保存了**栈帧（Frame）**，每个栈帧对应一个方法的调用。简单来说，每当方法被调用时，就会创建一个新的栈帧，当方法执行完毕后，这个栈帧就会被移除。

线程上下文切换（Thread Context Switch）：

* **线程上下文切换**是指 CPU 从一个线程切换到另一个线程。

  这通常发生在以下情况下：

  1. **线程的 CPU 时间片用完了**：操作系统会给每个线程分配一小段 CPU 时间，这叫时间片。当一个线程的时间片用完时，CPU 会切换到另一个线程。
  2. **有更高优先级的线程需要运行**：如果一个新线程有更高的优先级，CPU 可能会立即切换到这个新线程。
  3. **线程自己调用了某些方法**：比如 `sleep()`（线程睡眠）、`yield()`（让出 CPU）、`wait()`（等待另一个线程通知）等，这些方法会让线程暂时停止执行，CPU 会去执行其他线程。
  4. **垃圾回收**：JVM 的垃圾回收线程有时需要暂停所有其他线程，以便清理内存。

程序计数器（Program Counter Register）：

- **程序计数器**是每个线程私有的一块小内存区域，它用于记住当前线程下一条 JVM 指令的执行地址。
- 当 CPU 切换线程时（上下文切换），需要保存当前线程的程序计数器和栈信息，并恢复另一个线程的状态。这就好比我们在打游戏时按下暂停键，再切换到另一个游戏继续玩一样，游戏会保存当前的状态。

JVM 规范并没有限定线程模型，以 HotSopot 为例：

- **Java 的线程是内核级线程（1:1 线程模型）**：每个 Java 线程都直接映射到一个操作系统的原生线程。这意味着 Java 的线程依赖于操作系统来调度和管理。
- 由于线程切换涉及从**用户态**切换到**内核态**（进入操作系统内核来调度线程），这是一个比较重的操作，会消耗一些系统资源，导致性能下降。

主线程和子线程：


- 在 Java 程序中，当你运行 `main` 方法时，JVM 会启动一个主线程，叫做 **main 线程**。
- 在 `main` 方法中，你可以创建其他线程，这些线程叫做 **子线程**。`main` 线程是所有子线程的“父线程”。

**总结**

简而言之，Java 线程在 JVM 中是一种轻量级的子任务，运行在 JVM 分配的独立栈空间中。当多个线程运行时，操作系统负责调度它们的执行顺序和时间。线程的切换和管理是一个复杂的过程，因为它涉及到用户态和内核态的转换，会消耗一定的系统资源。
...
![](./Images/Prog_Image/JUC-%E7%BA%BF%E7%A8%8B6%E7%A7%8D%E7%8A%B6%E6%80%81.png)
...
* Mark Word 结构：最后两位是**锁标志位**
  ...
  ![](./Images/Prog_Image/JUC-Monitor-MarkWord%E7%BB%93%E6%9E%8432%E4%BD%8D.png)

* 64 位虚拟机 Mark Word：

  ![](./Images/Prog_Image/JUC-Monitor-MarkWord%E7%BB%93%E6%9E%8464%E4%BD%8D.png)
...
* 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把**对象原有的 MarkWord 存入线程栈中的锁记录**中（轻量级锁部分详解）
  <img src="./Images/Prog_Image/JUC-Monitor%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%861.png" style="zoom:67%;" />
* 在 Thread-2 上锁的过程，Thread-3、Thread-4、Thread-5 也执行 synchronized(obj)，就会进入 EntryList BLOCKED（双向链表）
* Thread-2 执行完同步代码块的内容，根据 obj 对象头中 Monitor 地址寻找，设置 Owner 为空，把线程栈的锁记录中的对象头的值设置回 MarkWord
* 唤醒 EntryList 中等待的线程来竞争锁，竞争是**非公平的**，如果这时有新的线程想要获取锁，可能直接就抢占到了，阻塞队列的线程就会继续阻塞
* WaitSet 中的 Thread-0，是以前获得过锁，但条件不满足进入 WAITING 状态的线程（wait-notify 机制）

![](./Images/Prog_Image/JUC-Monitor%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%862.png)
...
![](./Images/Prog_Image/JUC-%E9%94%81%E5%8D%87%E7%BA%A7%E8%BF%87%E7%A8%8B.png)
...
<img src="./Images/Prog_Image/JUC-Monitor-MarkWord%E7%BB%93%E6%9E%8464%E4%BD%8D.png" style="zoom: 67%;" />
...
* 创建锁记录（Lock Record）对象，每个线程的**栈帧**都会包含一个锁记录的结构，存储锁定对象的 Mark Word
  ...
  ![](./Images/Prog_Image/JUC-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%861.png)

* 让锁记录中 Object reference 指向锁住的对象，并尝试用 CAS 替换 Object 的 Mark Word，将 Mark Word 的值存入锁记录
  
* 如果 CAS 替换成功，对象头中存储了锁记录地址和状态 00（轻量级锁） ，表示由该线程给对象加锁
  ![](./Images/Prog_Image/JUC-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%862.png)
...
* 如果 CAS 失败，有两种情况：
  ...
  * 如果是线程自己执行了 synchronized 锁重入，就添加一条 Lock Record 作为重入的计数
  ...
  ![](./Images/Prog_Image/JUC-%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%863.png)
...
* 当 Thread-1 进行轻量级加锁时，Thread-0 已经对该对象加了轻量级锁
  ...
  ![](./Images/Prog_Image/JUC-%E9%87%8D%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%861.png)

* Thread-1 加轻量级锁失败，进入锁膨胀流程：为 Object 对象申请 Monitor 锁，**通过 Object 对象头获取到持锁线程**，将 Monitor 的 Owner 置为 Thread-0，将 Object 的对象头指向重量级锁地址，然后自己进入 Monitor 的 EntryList BLOCKED

  ![](./Images/Prog_Image/JUC-%E9%87%8D%E9%87%8F%E7%BA%A7%E9%94%81%E5%8E%9F%E7%90%862.png)
...
* 自旋成功的情况：
      <img src="./Images/Prog_Image/JUC-%E8%87%AA%E6%97%8B%E6%88%90%E5%8A%9F.png" style="zoom: 80%;" />

* 自旋失败的情况：

  <img src="./Images/Prog_Image/JUC-%E8%87%AA%E6%97%8B%E5%A4%B1%E8%B4%A5.png" style="zoom:80%;" />
    ...
    public static void main(String[] args) {
        new Thread(()->{
            ...
            synchronized (resources1){
                ...
                try {
                    Thread.sleep(2000);// 休息两秒，防止线程1直接运行王超
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                // 2秒内线程2肯定可以锁住资源2
        ...
        new Thread(()->{
            ...
            synchronized (resources2){
                ...
                try {
                    Thread.sleep(2000);// 休息两秒，防止线程2直接运行王超
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                // 2秒内线程1肯定可以锁住资源1
...
![](./Images/Prog_Image/JUC-park%E5%8E%9F%E7%90%861.png)
...
* 先 unpark：
  ...
  ![](./Images/Prog_Image/JUC-park%E5%8E%9F%E7%90%862.png)
...
![](./Images/Prog_Image/JUC-%E4%BF%9D%E6%8A%A4%E6%80%A7%E6%9A%82%E5%81%9C.png)
...
![](./Images/Prog_Image/JUC-%E4%BF%9D%E6%8A%A4%E6%80%A7%E6%9A%82%E5%81%9C%E5%A4%9A%E4%BB%BB%E5%8A%A1%E7%89%88.png)
...
![](./Images/Prog_Image/JUC-%E7%94%9F%E4%BA%A7%E8%80%85%E6%B6%88%E8%B4%B9%E8%80%85%E6%A8%A1%E5%BC%8F.png)
...
![](./Images/Prog_Image/JMM%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B.png)
...
<img src="./Images/Prog_Image/JMM-%E5%86%85%E5%AD%98%E4%BA%A4%E4%BA%92.png" style="zoom: 67%;" />
...
![](./Images/Prog_Image/JMM-%E5%8F%AF%E8%A7%81%E6%80%A7%E4%BE%8B%E5%AD%90.png)
...
<img src="./Images/Prog_Image/JMM-CPU%E7%BC%93%E5%AD%98%E7%BB%93%E6%9E%84.png" style="zoom: 50%;" />
...
<img src="./Images/Prog_Image/JUC-%E5%86%85%E5%AD%98%E4%BC%AA%E5%85%B1%E4%BA%AB.png" style="zoom: 67%;" />
...
<img src="./Images/Prog_Image/JUC-%E7%BC%93%E5%AD%98%E4%B8%80%E8%87%B4%E6%80%A7.png" style="zoom:80%;" />
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，从主存刷新变量值，加载的是主存中最新数据
  ...
  <img src="./Images/Prog_Image/JMM-volatile%E4%BF%9D%E8%AF%81%E5%8F%AF%E8%A7%81%E6%80%A7.png" style="zoom:67%;" />
...
* 有序性的保证也只是保证了本线程内相关代码不被重排序
  ...
  <img src="./Images/Prog_Image/JMM-volatile%E4%B8%8D%E8%83%BD%E4%BF%9D%E8%AF%81%E5%8E%9F%E5%AD%90%E6%80%A7.png" style="zoom:67%;" />
...
![](./Images/Prog_Image/JMM-DCL%E5%87%BA%E7%8E%B0%E7%9A%84%E9%97%AE%E9%A2%98.png)
...
![](./Images/Prog_Image/JUC-ThreadLocal%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84JDK8%E5%89%8D.png)
...
![](./Images/Prog_Image/JUC-ThreadLocal%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84JDK8%E5%90%8E.png)
...
* set()：添加数据，ThreadLocalMap 使用**线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  ![](./Images/Prog_Image/JUC-replaceStaleEntry%E6%B5%81%E7%A8%8B.png)
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  <img src="./Images/Prog_Image/JUC-ThreadLocal%E6%8E%A2%E6%B5%8B%E5%BC%8F%E6%B8%85%E7%90%861.png" style="zoom:67%;" />

  <img src="./Images/Prog_Image/JUC-ThreadLocal%E6%8E%A2%E6%B5%8B%E5%BC%8F%E6%B8%85%E7%90%862.png" style="zoom:67%;" />
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  <img src="./Images/Prog_Image/JUC-ThreadLocal%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E5%BC%BA%E5%BC%95%E7%94%A8.png" style="zoom:67%;" />
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  <img src="./Images/Prog_Image/JUC-ThreadLocal%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E5%BC%B1%E5%BC%95%E7%94%A8.png" style="zoom:67%;" />
...
* 当一个节点入队：
  ...
  ![](./Images/Prog_Image/JUC-LinkedBlockingQueue%E5%85%A5%E9%98%9F%E6%B5%81%E7%A8%8B.png)
...
* `h = head` → `first = h.next` 
  ...
  ![](./Images/Prog_Image/JUC-LinkedBlockingQueue%E5%87%BA%E9%98%9F%E6%B5%81%E7%A8%8B1.png)

* `h.next = h` → `head = first`

  ![](./Images/Prog_Image/JUC-LinkedBlockingQueue%E5%87%BA%E9%98%9F%E6%B5%81%E7%A8%8B2.png)
...
1. corePoolSize (核心线程数)：
   ...
   - 定义了线程池在没有空闲线程时，能够保持的最小线程数，即使这些线程是空闲的。

   - 即使线程池中的线程数小于 corePoolSize，只要有新的任务提交，线程池都会创建新线程来处理任务，而不论 workQueue 是否有任务。

   - 补充：在 allowCoreThreadTimeOut(true) 被设置后，核心线程也会受到 keepAliveTime 的限制。

2. maximumPoolSize (最大线程数)：

   - 线程池能够容纳的最大线程数，当 `workQueue` 满了且仍有新任务时，线程池会创建新的线程直到线程数达到 `maximumPoolSize`。

   - 当 `corePoolSize` 线程数被占满，且任务队列也满了，才会创建额外的救急线程来执行任务。

   - **补充**：此参数只在 `workQueue` 为有界队列时有用，因为如果 `workQueue` 是无界的，线程池会优先将任务放入队列，通常不会超过 `corePoolSize`。

3. keepAliveTime：

   - 指定非核心线程在没有任务执行时等待新任务的最长时间，超过这个时间会被终止。


   - 默认情况下，这个值只对超过 `corePoolSize` 的救急线程有效。


   - **补充**：当 `allowCoreThreadTimeOut(true)` 被设置时，核心线程也会在空闲 `keepAliveTime` 后被销毁。

4. unit (时间单位)：

   * `keepAliveTime` 的时间单位，可以是 `TimeUnit.SECONDS`、`TimeUnit.MILLISECONDS` 等。

5. workQueue  (阻塞队列)：

   * 这是存放等待执行任务的队列，只有当核心线程池满了，新的任务才会被放入 `workQueue`。


     - 常见的 `BlockingQueue` 实现：

       - `LinkedBlockingQueue`：一个无界队列，适合执行大量任务时使用。

       - `ArrayBlockingQueue`：一个有界队列，适合控制队列大小，避免无限制地增加任务。

       - `SynchronousQueue`：一个不存储任务的队列，每个任务必须立即有线程执行，否则会尝试创建新线程。

6. threadFactory  (线程工厂)：

   * 用于创建新线程，通常用于自定义线程名称、优先级等。

   - 默认的 `Executors.defaultThreadFactory()` 会创建普通的用户线程，但可以通过自定义线程工厂来控制更多细节。

7. handler (拒绝策略)：

   * 当线程池的线程数量达到 `maximumPoolSize`，并且 `workQueue` 已满时，会调用 `RejectedExecutionHandler` 来处理拒绝的任务。
...
![](./Images/Prog_Image/JUC-%E7%BA%BF%E7%A8%8B%E6%B1%A0%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.png)
...
![](./Images/Prog_Image/JUC-newSingleThreadExecutor.png)
...
* 状态表示：
  ...
  ![](./Images/Prog_Image/JUC-%E7%BA%BF%E7%A8%8B%E6%B1%A0%E7%8A%B6%E6%80%81%E8%BD%AC%E6%8D%A2%E5%9B%BE.png)
...
* 请求的共享资源被占用，AQS 用队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  <img src="./Images/Prog_Image/JUC-AQS%E5%8E%9F%E7%90%86%E5%9B%BE.png" style="zoom: 80%;" />
...
* 使用了 FIFO 先入先出队列，并不支持优先级队列，**同步队列是双向链表，便于出队入队**
  ...
  ![](./Images/Prog_Image/JUC-AQS%E9%98%9F%E5%88%97%E8%AE%BE%E8%AE%A1.png)
...
<img src="./Images/Prog_Image/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%811.png" style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列（不是阻塞队列），前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  <img src="./Images/Prog_Image/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%812.png" style="zoom:80%;" />
...
* 再有多个线程经历竞争失败后：
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%813.png)
...
* 唤醒的线程会从 park 位置开始执行，如果加锁成功（没有竞争），会设置
  ...
  * 原本的 head 因为从链表断开，而可被垃圾回收（图中有错误，原来的头节点的 waitStatus 被改为 0 了）
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%814.png)
...
* 如果这时有其它线程来竞争**（非公平）**，例如这时有 Thread-4 来了并抢占了锁
  ...
  * Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%815.png)
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里**不存在并发**
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F1.png)
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F2.png)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  ![](./Images/Prog_Image/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F3.png)
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  <img src="./Images/Prog_Image/JUC-ReentrantReadWriteLock%E7%BC%93%E5%AD%98.png" style="zoom:80%;" />
...
public class TestGenericDao {
    public static void main(String[] args) {
        ...
        int empno = 1;
        Course emp = dao.queryOne(Course.class, sql, empno);
        System.out.println(emp);
        emp = dao.queryOne(Course.class, sql, empno);
        System.out.println(emp);
        emp = dao.queryOne(Course.class, sql, empno);
        System.out.println(emp);

        System.out.println("============> 更新");
        dao.update("update course set c_name = ? where c_id = ?", "语文", empno);
        emp = dao.queryOne(Course.class, sql, empno);
        System.out.println(emp);
    }
}
...
>   - 只适合单机
>
>   - 并发性还是低，目前只会用一把锁 (可以将锁再细分，这样并发度就会很高)
>
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  <img src="./Images/Prog_Image/JUC-ReentrantReadWriteLock%E5%8A%A0%E9%94%811.png" style="zoom: 80%;" />

* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子

  ![](./Images/Prog_Image/JUC-ReentrantReadWriteLock%E5%8A%A0%E9%94%812.png)
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  <img src="./Images/Prog_Image/JUC-ReentrantReadWriteLock%E8%A7%A3%E9%94%811.png" style="zoom: 67%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  <img src="./Images/Prog_Image/JUC-ReentrantReadWriteLock%E8%A7%A3%E9%94%812.png" style="zoom: 67%;" />
...
<img src="./Images/Prog_Image/JUC-CyclicBarrier%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.png" style="zoom: 80%;" />
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ![](./Images/Prog_Image/JUC-Semaphore%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B1.png)
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  ![](./Images/Prog_Image/JUC-Semaphore%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B2.png)
...
![ConcurrentHashMap数据结构](./Images/Prog_Image/ConcurrentHashMap%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84.png)
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ![](./Images/Prog_Image/JUC-ConcurrentHashMap-LastRun%E6%9C%BA%E5%88%B6.png)
...
* 缺点：Segments 数组默认大小为16，这个容量初始化指定后就不能改变了，并且不是懒惰初始化
  ...
  ![](./Images/Prog_Image/JUC-ConcurrentHashMap%201.7%E5%BA%95%E5%B1%82%E7%BB%93%E6%9E%84.png)
...
<img src="./Images/Prog_Image/JUC-CopyOnWriteArrayList%E5%BC%B1%E4%B8%80%E8%87%B4%E6%80%A7.png" style="zoom:80%;" />
...
![](./Images/Prog_Image/JUC-ConcurrentSkipListMap%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84.png)
...
* findPredecessor()：寻找前置节点
  ...
  ![](./Images/Prog_Image/JUC-ConcurrentSkipListMap-Put%E6%B5%81%E7%A8%8B.png)
...
* remove()
  ...
  ![](./Images/Prog_Image/JUC-ConcurrentSkipListMap-remove%E6%B5%81%E7%A8%8B.png)
...
![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C1.png)

![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C2.png)

![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C3.png)
...
![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C1.png)

![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C2.png)

![](./Images/Prog_Image/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C3.png)
...
![](./Images/Prog_Image/IO%E6%A8%A1%E5%9E%8B-%E9%98%BB%E5%A1%9E%E5%BC%8FIO.png)
...
![](./Images/Prog_Image/IO%E6%A8%A1%E5%9E%8B-%E9%9D%9E%E9%98%BB%E5%A1%9E%E5%BC%8FIO.png)
...
![](./Images/Prog_Image/IO%E6%A8%A1%E5%9E%8B-%E4%BF%A1%E5%8F%B7%E9%A9%B1%E5%8A%A8IO.png)
...
![](./Images/Prog_Image/IO%E6%A8%A1%E5%9E%8B-IO%E5%A4%8D%E7%94%A8%E6%A8%A1%E5%9E%8B.png)
...
![](./Images/Prog_Image/IO%E6%A8%A1%E5%9E%8B-%E5%BC%82%E6%AD%A5IO%E6%A8%A1%E5%9E%8B.png)
...
![](./Images/Prog_Image/IO-select%E8%B0%83%E7%94%A8%E8%BF%87%E7%A8%8B.png)
...
![](./Images/Prog_Image/IO-%E7%94%A8%E6%88%B7%E6%80%81%E5%92%8C%E5%86%85%E6%A0%B8%E6%80%81.png)
...
![](./Images/Prog_Image/IO-%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8%E7%9A%84%E8%BF%87%E7%A8%8B.jpg)
...
<img src="./Images/Prog_Image/IO-DMA.png" style="zoom: 50%;" />
...
![](./Images/Prog_Image/IO-BIO%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)

read 调用图示：read、write 都是系统调用指令

<img src="./Images/Prog_Image/IO-%E7%BC%93%E5%86%B2%E5%8C%BA%E8%AF%BB%E5%86%99.png" style="zoom: 67%;" />
...
![](./Images/Prog_Image/IO-mmap%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)
...
![](./Images/Prog_Image/IO-sendfile%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)
...
<img src="./Images/Prog_Image/%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B.png" alt="三次握手" style="zoom: 50%;" />

<img src="./Images/Prog_Image/%E5%9B%9B%E6%AC%A1%E6%8C%A5%E6%89%8B.png" alt="四次挥手" style="zoom: 67%;" />
...
* 常用 API：`public Socket accept()`，**阻塞等待**接收一个客户端的 Socket 管道连接请求，连接成功返回一个 Socket 对象
  ...
  <img src="./Images/Prog_Image/Netty-TCP%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B.png" style="zoom:67%;" />
...
![](./Images/Prog_Image/BIO%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6.png)

![](./Images/Prog_Image/TCP-%E5%B7%A5%E4%BD%9C%E6%A8%A1%E5%9E%8B.png)
...
![](./Images/Prog_Image/NIO%E6%A1%86%E6%9E%B6.png)
...
![](./Images/Prog_Image/NIO-Buffer.png)
...
* 位置、限制、容量遵守以下不变式： **0 <= position <= limit <= capacity**
  ...
  <img src="./Images/Prog_Image/NIO-Buffer%E6%93%8D%E4%BD%9C.png" style="zoom:67%;" />
...
| public final boolean hasRemaining()         | 判断缓冲区中 `position` 和 `limit` 之间是否存在未处理的元素  |
...
public class TestBuffer {
    ...
    public void test(){
        ...
        buffer.get(dst);
        System.out.println(dst.length);// 7
        System.out.println(new String(dst, 0, dst.length));// seazean
        System.out.println(buffer.position());//7
        System.out.println(buffer.limit());//7
...
<img src="./Images/Prog_Image/JVM-%E7%9B%B4%E6%8E%A5%E5%86%85%E5%AD%98%E7%9B%B4%E6%8E%A5%E7%BC%93%E5%86%B2%E5%8C%BA.png" style="zoom: 50%;" />

<img src="./Images/Prog_Image/JVM-%E7%9B%B4%E6%8E%A5%E5%86%85%E5%AD%98%E9%9D%9E%E7%9B%B4%E6%8E%A5%E7%BC%93%E5%86%B2%E5%8C%BA.png" style="zoom:50%;" />
...
![](./Images/Prog_Image/NIO-%E5%A4%8D%E5%88%B6%E6%96%87%E4%BB%B6.png)
...
![](./Images/Prog_Image/NIO-Selector.png)
